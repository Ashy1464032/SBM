...
export default function MayorBox({name, perks}: MayorProps) {
    return (
        <div className="mt-20 mx-auto border-4 border-stone-600 rounded-lg border-double shadow-2xl w-[50ch] justify-center">
            ...
            <ol>
                {perks && perks.map((perk, index) => (
                    <li key={index}>
                        {index + 1}. {perk.name}
                        <p className="max-w-[50ch] text-center" style={{textAlign: 'center'}}>{perk.description}</p>
                    </li>
                ))}
            </ol>
        </div>
